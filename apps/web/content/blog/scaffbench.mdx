...
description: "72 runs across Claude Code and Codex CLI — six models, three creation paths, four project specs. We measured wall-clock time, output tokens, dollar cost — and whether the generated project actually installs and builds."
date: 2026-06-12
authors:
...
specs, same prompts, same validation.
...
- **One run per cell.** 72 runs is enough to see the structure, not to put confidence intervals on
...
- **The sweeps aren't head-to-head.** The GPT runs used a different agent CLI (Codex), different
  reasoning-effort settings, an earlier generator version, and report no dollar cost. Within-sweep
  path comparisons are sound; cross-vendor model rankings are not what this benchmark measures.
