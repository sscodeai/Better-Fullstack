...
- `--verify` runs generated project checks after scaffolding without starting dev servers.
...
| `--part` | Add a multi-ecosystem stack part, e.g. `frontend:typescript:next` or `backend.orm:go:gorm`. Repeat for each selected app or capability. |
...
| `--verify` | Run generated backend checks after scaffolding without starting dev servers: Go `go test`, Rust `cargo check`, Python `ruff`, and Elixir compile when available. |
...
Repeat `--part` to bind each generated app or owned capability.
